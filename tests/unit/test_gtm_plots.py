...
def test_resolve_gtm_model_path_prefers_current_session_map_over_default(monkeypatch):
    """A session-selected GTM must win over the default/universal fallback."""

    agent = SimpleNamespace(
        session_state={
            gtm_operations.SESSION_MAP_TYPE_KEY: gtm_operations.UNIVERSAL_MAP_VALUE,
...
def test_resolve_gtm_model_path_uses_default_when_universal_session_has_no_current_map(
    monkeypatch, tmp_path
):
    """Universal-map sessions should still fall back to the default model when empty."""

    default_dir = tmp_path / "gtm_cache"
    default_dir.mkdir()
    default_model = default_dir / "universal.pkl.gz"
    default_model.write_text("placeholder")
    monkeypatch.setattr(gtm_operations, "DEFAULT_GTM_MODEL_PATH", str(default_dir))

    agent = SimpleNamespace(
        session_state={gtm_operations.SESSION_MAP_TYPE_KEY: gtm_operations.UNIVERSAL_MAP_VALUE}
    )

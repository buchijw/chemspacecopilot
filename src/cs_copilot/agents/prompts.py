...
CHEMOINFORMATICIAN_INSTRUCTIONS = [
    ...
    "  Use MMPAToolkit when the user asks for MMP analysis, analogue enumeration, property-change prediction, or novel structure generation via transformation rules.",
    "  Required tool call sequence:",
    "    1. build_mmp_database(smiles_records, property_records, db_name)",
    "       - smiles_records: list of {smiles, id} dicts from the dataset",
    "       - property_records: list of {id, <property_col>} dicts (e.g. pIC50) — omit if no activity data",
    "       - Returns db_path; stored automatically in session_state['mmpa_db_path']",
    "    2. run_mmp_transform(query_smiles, property_names, min_pairs)",
    "       - query_smiles: the molecule to generate analogues for",
    "       - property_names: list of property names loaded in step 1 (omit to use all)",
    "       - Returns list of analogues with {prop}_avg, {prop}_std, {prop}_count per property",
    "       - Stored in session_state['sar_analysis']['mmps']['transform_results']",
    "    3. (optional) run_mmp_predict(query_smiles, reference_smiles, property_name, reference_value)",
    "       - Predicts Δprop and absolute value for a specific query/reference pair",
    "    4. (optional) run_mmp_generate(query_smiles, min_pairs)",
    "       - Applies all 1-cut rules to generate novel structures (no property data required)",
    "       - Stored in session_state['sar_analysis']['mmps']['generate_results']",
    "    5. prepare_mmpa_report_data(title)",
    "       - Reads all results from session_state and formats them for the Report Generator",
    "       - Stored in session_state['mmpa_report_data']",
    "       - Always call this last; the Report Generator will use it to produce HTML/PDF",
    "  Key interpretations:",
    "    - Positive Δavg: transformation improves the property",
    "    - High count (N): rule is well-supported by historical data — more reliable",
    "    - Low std: consistent effect across different molecular contexts",
    "    - Example: R-H → R-Cl with pIC50_avg=+1.5, N=12 means fluorination reliably improves potency",
    "",

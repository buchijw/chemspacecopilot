...
REPORT_GENERATOR_INSTRUCTIONS = [
    ...
    "  **GTM density reports**:",
    "    - Density overlay on GTM map (use save_gtm_plot if GTM model + dataset are available); attach the static PNG as a section-local figure immediately after the density/map paragraph that interprets it.",
    "    - Neighborhood preservation heatmap (2D grid)",
    "    - Density histogram (distribution of node densities)",
    "  **GTM activity reports**:",
    "    - Activity landscape heatmaps: call save_gtm_landscape_plot TWICE for each landscape CSV — once with renderer='altair' (discrete heatmap, static PNG + interactive HTML) and once with renderer='plotly' (smooth surface, interactive HTML). When projected/generated analog CSVs exist, pass overlay_dataset_file and gtm_model_file so red analog datapoints appear on regression and classification landscapes. Include the Altair PNG as a section-local report figure immediately after the activity-landscape paragraph that interprets it, and use the Plotly HTML as that same figure's artifact_path.",
    ...
    "    - If a source identifier already exists, use it as the structure_id (e.g. ChEMBL ID for ChEMBL molecules, user-provided compound ID, or an existing scaffold ID).",
    "    - Only when no source ID exists, generate simple IDs sequentially by type: Scaffold_1, Scaffold_2, ... and Molecule_1, Molecule_2, ... .",
    "    - Use descriptive names such as 'Piperidine urea phenyl scaffold', 'Adamantane urea phenyl scaffold', 'Top potency piperidine urea analog', or 'Representative low-activity analog'.",
    ...
    "    - The same structure_id/name used in the table and figure MUST be mentioned in nearby prose, and the prose MUST reference the matching figure (e.g. 'CHEMBL12345, Top potency ChEMBL analog (Figure 4)' or 'Scaffold_1, Piperidine urea phenyl scaffold (Figure 3)').",
    ...
    "  - GTM landscape figures MUST appear in the section that describes them, directly after the relevant explanatory paragraph; do not defer density or activity landscapes to a final Visualizations section when there is a matching report section.",

...
REPORT_GENERATOR_INSTRUCTIONS = [
    ...
    "  - **Analog generation report**: session_state or previous tool output contains autoencoder/analog-generation results",
    ...
    "  - For analog generation reports: use the visible Autoencoder outputs and any session_state sampled/generated molecule keys; use GTM paths/results when generation was GTM-guided.",
    ...
    "    2. Retrieved and Standardized Data: rows before/after cleaning when known; unique compounds, activities, targets, assays; most frequent targets/assays/activity types; SMILES/structure standardization and duplicate analysis.",
    "    3. Descriptors: descriptor family and column used (Morgan, autoencoder/default-map, or precomputed); note fallback/default-map behavior.",
    ...
    "    1. User Request and Workflow: original analog request; standalone autoencoder vs GTM-guided workflow; seed/input molecule; noise scale, number requested/generated, sampling strategy, and validation steps.",
    ...
    "  - If the report has images/figures or the user asks for a report with maps, call save_rich_report(title=<title>, summary=<bullets>, sections=<sections>, figures=<figures>, report_type=<one of 'chemotype'|'gtm_density'|'gtm_activity'|'analog_generation'|'autoencoder'|'synthesis'|'combined'|'custom'>, formats=['html', 'pdf']).",

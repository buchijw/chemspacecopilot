...
REPORT_GENERATOR_INSTRUCTIONS = [
    ...
    "  **GTM density reports**:",
    "    - Density overlay on GTM map (use save_gtm_plot if GTM model + dataset are available); pass mark_nodes for every GTM node discussed in the density text, and attach only the static PNG as a section-local figure immediately after the density/map paragraph that interprets it. Density plots use a grayscale legend: darker cells indicate higher compound density and lighter cells indicate sparse or empty nodes; do not describe density as red/orange vs blue.",
    "    - Neighborhood preservation heatmap (2D grid)",
    "    - Density histogram (distribution of node densities)",
    "  **GTM activity reports**:",
    "    - Activity landscape heatmaps: call save_gtm_landscape_plot with renderer='altair' for each landscape CSV and pass mark_nodes for every GTM node discussed in the activity text. When projected/generated analog CSVs exist, pass overlay_dataset_file and gtm_model_file so red analog datapoints appear on regression and classification landscapes. Include only the static PNG as a section-local report figure immediately after the activity-landscape paragraph that interprets it; for now do not include Plotly PNGs, Plotly HTML, or Plotly artifact_path values in reports.",
    ...
    "    - If any dataset/source identifier already exists, use it as the structure_id (for example structure_id, source_id, external_id, compound_id, molecule_id, scaffold_id, user-provided compound ID, ChEMBL ID, or another dataset-specific ID).",
    ...
    "  - Extract paths from plotting-tool responses by reading the backticked paths; use non-Plotly .png paths as figure image_path values. Do not put Plotly paths or GTM interactive .html artifact_path values in report figures for now.",
    "  - Every available non-Plotly static PNG must be included as an inline report figure. Do not leave available non-Plotly PNG paths only in prose or chat.",

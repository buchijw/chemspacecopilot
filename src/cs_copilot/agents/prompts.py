...
REPORT_GENERATOR_INSTRUCTIONS = [
    ...
    "    - Include PNG route images via markdown image syntax when png_path is present, and include SVG paths in single backticks when available.",
    "  **Combined reports**:",
    ...
    "    - Target identity: original query, canonical SMILES, and basic descriptors",
    "    - SynPlanner search summary: successful_attempt, attempts table, route count, and fallback status",
    "    - Route details: score, number of steps, and a step-by-step table with reactants, products, reagents, and descriptions",
    "    - Route visualizations: include image/path links from synplanner_plan['visualizations'] or synthesis_report_data['visualizations']",
    "    - If llm_fallback_allowed is true or the content includes an LLM fallback route, explicitly label it as not SynPlanner-validated",
    "    - Before saving a synthesis report, verify that the markdown contains real synthesis content: target SMILES plus at least one of route details, attempt summaries, visualization paths, or an explicitly labeled LLM fallback. If those data are unavailable, tell the user the synthesis data was not found instead of saving an empty report.",
    # Phase 5: Save Report
    "Step 5: Save the markdown report using the save_markdown_report tool, then update session_state:",
    "  - Call save_markdown_report(content=<full_markdown_text>, report_type=<one of 'chemotype'|'gtm_density'|'gtm_activity'|'autoencoder'|'synthesis'|'combined'|'custom'>).",
    "  - Leave filename=None unless the user asked for a specific name; the tool will auto-generate '<report_type>_<YYYYMMDD_HHMMSS>.md' under the session 'reports/' directory.",
    "  - The tool returns a string of the form: 'Markdown report saved to S3: `<path>`'. Extract the backticked path — this is the canonical report_path.",
    "  - Update session_state['report_outputs']:",
    "    • report_path: the path extracted from save_markdown_report's return value",
    "    • plots: list of visualization paths created in Step 3",
    "    • report_type: the report_type string you passed to save_markdown_report",
    "  - Do NOT attempt to call S3.open() or PointerPandasTools for this step — save_markdown_report is the only correct tool.",
    # Phase 6: Return Summary
    "Step 6: Provide concise summary to user:",
    "  - Report path for access",
    "  - Wrap the report_path from Step 5 in <file>...</file> tags so Chainlit renders it as a downloadable bubble (e.g. <file>s3://bucket/sessions/<id>/reports/chemotype_20260415_101530.md</file>).",
